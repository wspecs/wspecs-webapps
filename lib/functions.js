 ...
 * @return {void}
 */
function respond(res, status, msg, trace){
 ...
 */
module.exports.IsValidSession = function(stamp){
 ...
 */
module.exports.caseIns = function(){
 ...
 * lowercased.
 */
module.exports.capFirst = function(){
 ...
 *  than 1
 */
module.exports.plurialize = function(text, len, plural) {
  if (len && len !== 0) {
 ...
 * @return {void}
 */
module.exports.responseHandler = function(err){
  if (err)
    log.error(err);
};
 ...
 * }} - paginatino info
 */
module.exports.paginate = function(arr, page, slice, reverse, hidePages){
  ...
  result.pages = getPages(page, slice, count, 7);
  return result;
};
 ...
 */
module.exports.peak = function(arr, limit){
...
module.exports.send = function(data, res, lastMod) {
  if (data) {
    data.success = true;
  }
 ...
 */
module.exports.mongoOrArray = function(key, values) {
  var expression = {$or: []};
...
/**
 * @param {Function} fn the function to rery
 * @param {number} [count=2] how many time to retry
 * @return {number} how many attempts were skipped.
 */
function retry(fn, count=2) {
  let tries = 0;
  if (count > 0) {
    try {
      ++tries;
...
module.exports.resolve = function(err, res, body) {
  if (err) {
    res.statusCode = 500;
...
module.exports.reject = function(error, rejectFn) {
  log.error(error);
  rejectFn(error);
};
 ...
 */
module.exports.chunk = (arr, chunkSize) => {
  const result  = [];
...
Array.prototype.chunk = module.exports.chunk;
